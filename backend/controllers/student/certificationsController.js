...
const tableName = 'student_certifications';
const sectionName = 'certifications';
...
const getCertifications = async (req, res) => {
  try {
    ...
    const query = `select * from ${tableName} where usn = $1 order by issue_date desc`;
...
const updateCertifications = async (req, res) => {
  try {
        ...
        for (const item of mappedData) {
            ...
            for (const key of Object.keys(item)) {
                if (validColumns.includes(key)) {
                    ...
                    if (['issue_date', 'expiry_date'].includes(key)) {
    ...
    try {
      ...
      const result = await client.query(`select * from ${tableName} where usn = $1 order by issue_date desc`, [usn]);
      res.json(mapData(sectionName, result.rows, 'fromDb'));
    } catch (e) {
