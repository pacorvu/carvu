...
const Events = () => {
  ...
  const formatCTC = (ctcStructure) => {
    ...
    // Calculate final CTC if not present or use available field
    const finalCTC = ctcStructure.final || ctcStructure.package || '0';
    
    // Detailed structure for tooltip
    // Structure: min - max + var pay % + stock options
  ...
  const formatEligibility = (eligibility) => {
    ...
    // Format: "Min CGPA: 7.5, Backlogs: 0"
    const parts = [];
    if (eligibility.min_cgpa) parts.push(`Min CGPA: ${eligibility.min_cgpa}`);
    if (eligibility.backlogs_allowed !== undefined) parts.push(`Backlogs: ${eligibility.backlogs_allowed}`);
    if (eligibility.allowed_programs && eligibility.allowed_programs.length > 0) {
       parts.push(`Programs: ${eligibility.allowed_programs.length} selected`);
    }
    
    if (parts.length === 0) return JSON.stringify(eligibility);
    
    return (
       <Tooltip label={JSON.stringify(eligibility, null, 2)} hasArrow placement="top">
         <Text cursor="help" borderBottom="1px dashed" borderColor="gray.400">
           {parts.join(', ')}
         </Text>
       </Tooltip>
    );
  ...
  const columnGroups = [
    ...
    },
    {
      ...
      columns: [
        ...
        { id: 'year', label: 'Year' },
        { id: 'tpo', label: 'TPO' },
        { id: 'eligibility_academics', label: 'Requirements' },
      ],
    },
    {
      id: 'compensation',
  ...
  const handleSaveEvent = async () => {
    ...
    const payload = {
      ...
      specialization_id: newEvent.specialization_id === '' ? null : parseInt(newEvent.specialization_id),
      school_id: parseInt(newEvent.school_id),
      company_id: parseInt(newEvent.company_id),
      year: parseInt(newEvent.year),
      no_shortlisted: newEvent.no_shortlisted === '' ? null : parseInt(newEvent.no_shortlisted),
      ctc_structure: { 
  ...
  useEffect(() => {
    if (newEvent.program_id) {
        PlacementService.getSpecializations(newEvent.program_id)
            .then(data => {
                if (Array.isArray(data)) {
              ...
              <Table variant="simple">
                <Thead bg="gray.50">
                  <Tr>
                    ...
                    {visibleColumns.includes('ctc_structure') && <Th>CTC</Th>}
                    {visibleColumns.includes('stipend_structure') && <Th>Stipend</Th>}
                    ...
                    {visibleColumns.includes('eligibility_academics') && <Th>Requirements</Th>}
                    ...
                    filteredDrives.map((drive) => {
                      ...
                      return (
                        <Tr 
                          ...
                          _hover={{ bg: isHighlighted ? "yellow.200" : "gray.50", cursor: "pointer" }}
                          transition="background-color 0.3s"
                          onClick={() => navigate(`/placement/events/${drive.id}`)}
                        >
                          ...
                          {visibleColumns.includes('ctc_structure') && <Td>{formatCTC(drive.ctc_structure)}</Td>}
                          {visibleColumns.includes('stipend_structure') && <Td>{formatStipend(drive.stipend_structure)}</Td>}
                          ...
                          {visibleColumns.includes('eligibility_academics') && <Td>{formatEligibility(drive.eligibility_academics)}</Td>}
                          ...
                          {visibleColumns.includes('actions') && (
                            <Td>
                              <HStack spacing={2}>
                                <Button size="xs" leftIcon={<EditIcon />} onClick={(e) => { e.stopPropagation(); handleEditClick(drive); }}>Edit</Button>
                                <Button 
                                    size="xs" 
                                    colorScheme="blue" 
                                    onClick={(e) => { e.stopPropagation(); handleManageClick(drive); }}
                                >
              ...
              <VStack spacing={6} align="stretch">
                ...
                <Box bg="white" p={6} borderRadius="lg" shadow="sm" borderWidth="1px" borderColor="gray.200">
                  <HStack mb={5} spacing={3} borderBottomWidth="1px" pb={3} borderColor="gray.100">
                    ...
                    <Heading size="md" color="gray.700">Requirements</Heading>
                  </HStack>
                  <SimpleGrid columns={{ base: 1, md: 2 }} spacing={6}>
                    <FormControl isRequired>
                    ...
                    <FormControl>
                       ...
                       <Select name="specialization_id" value={newEvent.specialization_id} onChange={handleInputChange} placeholder="Select Specialization" bg="gray.50" _focus={{ bg: 'white', borderColor: 'red.500' }}>
                          {specializationList.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                       </Select>
                    </FormControl>
