...
const JobOffers = () => {
  ...
  const handleAddOffer = async () => {
    ...
    if (!newOffer.placement_drive_id) {
      toast({ title: "Select a placement drive", status: "warning" });
      return;
    }
    ...
    try {
      ...
      let successCount = 0;
      let failureCount = 0;
      const missingProcessUsns = [];

      for (const student of selectedStudents) {
        const processId = processMap.get(student.usn);

        if (!processId) {
          missingProcessUsns.push(student.usn);
          failureCount += 1;
          continue;
        }
  ...
  const filteredDrivesForModal = useMemo(() => {
    ...
    return formDrives.filter(drive => {
      const id = drive.id;
      if (!id) return false;
      return driveIdSets.every(set => set.has(id));
    });
  }, [formDrives, selectedStudents, studentDriveMap]);
                  ...
                  <VStack spacing={4} align="stretch">
                    ...
                    <FormControl isRequired>
                      ...
                      <Select
                        ...
                        value={newOffer.placement_drive_id}
