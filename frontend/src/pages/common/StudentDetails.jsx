...
const DashboardView = ({ applications, completionPercentage, showNotification, navigate, isReadOnly }) => {
  ...
  const getFinalStatusLabel = (app) => {
    const fs = app.final_select_status;
    if (fs === true) return "Shortlisted";
    if (fs === false) return "Rejected";
    return app.registration_status || "Applied";
  };

  const interviewsScheduled = applications.filter(app => app.interview_status === true).length;
  const offersReceived = applications.filter(app => app.final_select_status === true).length;

  const filteredApplications = applications.filter(app => {
    const companyName = app.company?.company_name || "Unknown Company";
    const role = app.drive?.job_type || "Role";
    const status = getFinalStatusLabel(app);
            ...
            {filteredApplications.length === 0 ? (
              ...
              filteredApplications.map((app) => (
                <JobCard 
                  ...
                  status={getFinalStatusLabel(app)} 
                  date={app.created_at} 
    ...
    try {
      ...
      if (profile) {
          ...
          const transformedStudent = {
              fullName: profile.personal?.fullName || `${profile.personal?.first_name || ''} ${profile.personal?.last_name || ''}`.trim() || 'N/A',
              usn: usn,
          ...
          return (
              <SectionContainer title="Career Overview">
                  ...
                  <HStack flexWrap="wrap">
                      {(Array.isArray(data.career?.keyExpertise)
                        ? data.career.keyExpertise
                        : (typeof data.career?.keyExpertise === 'string'
                            ? data.career.keyExpertise.split(',').map(s => s.trim()).filter(Boolean)
                            : [])
                      ).map((s,i) => <Badge key={i} colorScheme="green">{s}</Badge>)}
                  </HStack>
              </SectionContainer>
          );
       case 'internships':
        ...
        <Box bg="white" borderBottom="1px solid" borderColor="gray.200" px={8} py={4}>
             <Flex justify="space-between" align="center">
                <HStack spacing={4}>
                    ...
                    <Box>
                        <Heading size="md" color="#20343c">{student?.fullName || student?.name || "Student Dashboard"}</Heading>
